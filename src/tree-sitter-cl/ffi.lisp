...
(defvar *ts-ffi-directory*
  #.(make-pathname :name nil :type nil :version nil
                   :defaults (or *compile-file-truename* *load-truename*)))

(defun setup-library-search-paths ()
  (let ((lib-path (merge-pathnames "lib/" *ts-ffi-directory*)))
    (when (uiop:directory-exists-p lib-path)
      (pushnew lib-path cffi:*foreign-library-directories*
               :test #'equal)))
  (let ((homebrew-lib (pathname "/opt/homebrew/lib/")))
    (when (uiop:directory-exists-p homebrew-lib)
      (pushnew homebrew-lib cffi:*foreign-library-directories*
               :test #'equal))))
...
(defun ensure-tree-sitter-loaded ()
  (unless *tree-sitter-loaded*
    (handler-case
      ...
      (error ()
        (let ((names #+darwin '("libtree-sitter.dylib" "libtree-sitter.0.dylib")
                     #-darwin '("libtree-sitter.so" "libtree-sitter.so.0")))
              ...
              (dolist (name names)
                (let ((full (merge-pathnames name dir)))
                  (when (probe-file full)
                    (handler-case
                        (progn
                          (cffi:load-foreign-library (namestring full))
                          (setf *tree-sitter-loaded* t)
                          (return-from ensure-tree-sitter-loaded t))
                      (error () nil))))))))))))

(defun ensure-ts-wrapper-loaded ()
  "Try to load the wrapper library for by-value struct handling."
  (unless *ts-wrapper-loaded*
    ;; Try CFFI named library first
    (handler-case
        (progn
          (cffi:load-foreign-library 'ts-wrapper)
          (setf *ts-wrapper-loaded* t))
      (error ()
        ;; Fallback: search cffi:*foreign-library-directories* manually
        (let ((name #+darwin "libts-wrapper.dylib" #-darwin "libts-wrapper.so"))
          (dolist (dir cffi:*foreign-library-directories*)
            (when (pathnamep dir)
              (let ((full (merge-pathnames name dir)))
                (when (probe-file full)
...
(cffi:defcstruct ts-query-predicate-step
  (type :uint32)    ; 0=Done, 1=Capture, 2=String
  (value-id :uint32))

(cffi:defcfun ("ts_query_predicates_for_pattern" %ts-query-predicates-for-pattern) :pointer
  (query :pointer)
  (pattern-index :uint32)
  (step-count :pointer))

(defun ts-query-predicates-for-pattern (query pattern-index)
  "Get predicate steps for a pattern. Returns list of (type . value-id) pairs."
  (cffi:with-foreign-object (step-count :uint32)
    (let ((ptr (%ts-query-predicates-for-pattern query pattern-index step-count))
          (count (cffi:mem-ref step-count :uint32)))
      (when (and ptr (not (cffi:null-pointer-p ptr)) (> count 0))
        (loop :for i :below count
              :for step := (cffi:mem-aptr ptr '(:struct ts-query-predicate-step) i)
              :collect (cons (cffi:foreign-slot-value step '(:struct ts-query-predicate-step) 'type)
                             (cffi:foreign-slot-value step '(:struct ts-query-predicate-step) 'value-id)))))))

(cffi:defcfun ("ts_query_string_value_for_id" %ts-query-string-value-for-id) :pointer
  (query :pointer)
  (id :uint32)
  (length :pointer))

(defun ts-query-string-value-for-id (query id)
  "Get a string value by ID from a query."
  (cffi:with-foreign-object (length :uint32)
    (let ((ptr (%ts-query-string-value-for-id query id length)))
      (unless (cffi:null-pointer-p ptr)
        (cffi:foreign-string-to-lisp ptr :count (cffi:mem-ref length :uint32))))))

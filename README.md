...
Hemlock is a Common Lisp editor with a long history — born at CMU in the 1980s, it shipped inside CMUCL and later SBCL. This is an active modernization: dead backends removed, dependencies cleaned up, a tree-sitter integration for real syntax highlighting, and a Helix-style modal editing layer being built on top of the original architecture. The core is still the same battle-tested text model. The direction is forward.

**Status:** v0.2.0 — loads clean on SBCL, TTY backend fully working, modal editing in place.
